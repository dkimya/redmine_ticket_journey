module TicketJourneyHelper
  ...
  def ticket_journey_breadcrumb(active_tab)
    active_section_key = ticket_journey_active_section_key(active_tab)
    [
      ticket_journey_section_label(active_section_key),
      ticket_journey_tab_label(active_tab) || ticket_journey_section_label(active_section_key)
    ]
  end

  def ticket_journey_active_section_key(active_tab)
    requested_support_section = params[:support_section].to_s.presence&.to_sym
    return requested_support_section if ticket_journey_section_label(requested_support_section)
    ...
    when :sprint_delivery, :duration, :cycle_distribution, :flow_report, :priority_risk
      :sprint_delivery
    when :owner_returns, :owner_workload
      :owner_returns
    when :qa_returns
      :qa_returns
    when :bug_analysis
  ...
  def ticket_journey_section_label(section_key)
    {
  ...
  def ticket_journey_tab_label(tab_key)
    {
      pmo_control: 'PMO Control',
      sprint_delivery: 'Sprint Delivery',
      owner_returns: 'Team / Ticket Owner Performance',
      ...
      owner_workload: 'Owner Workload / Time Support',
      aging_risk: 'Aging / SLA Risk',
      priority_risk: 'Priority / SLA Risk',
  ...
  def ticket_journey_breadcrumb_tag(active_tab)
    section_label, view_label = ticket_journey_breadcrumb(active_tab)

    content_tag(:div, class: 'tj-dashboard-breadcrumb') do
      safe_join([
        content_tag(:span, section_label),
        content_tag(:span, '/', class: 'tj-dashboard-breadcrumb-separator'),
        content_tag(:strong, view_label)
      ])
    end
  end
